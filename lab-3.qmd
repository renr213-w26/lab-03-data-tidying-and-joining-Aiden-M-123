...
```
Country inflation has 44 rows and 30 columns. Each represents a year and each column represents a country.
...
The countries withe the highest inflation were Argentina (48.4), Turkey (19.6), and Brazil (8.3). All of which were substantially higher than the U.S inflation rate of 4.7.
...
New Zealand's inflation change was the largest at 3.06. The ratio is greater than 1 so inflation increased over this period.
...
glimpse(country_inflation_long)
```
...
```
The highest inflation rate was 2075.89 in Brazil in 1994.
...
```
The lowest inflation rate was -4.48 in Ireland in 2009.
...
```
The highest inflation rate was 2075.89 in Brazil in 1994. The lowest inflation rate was -4.48 in Ireland in 2009.
...
```{r}
countries_of_interest <- c("United States", "New Zealand", "Canada")
```

If they are Türkiye, United States, and China, you can express this as follows:

```{r}
#Example - # so it does not get executed
...
I chose the United States because it had high inflation, New Zealand had the largest change in inflation, and Canada is of interest because its my home country.
...
```{r}
inflation_countries <- country_inflation_long |>
...
```{r}
ggplot(inflation_countries, 
       aes(x = year, y = annual_inflation, color = country, shape = country)) +
       geom_line() +
  geom_point() +
       labs(
         title = "Annual Inflation for Selected Countries",
         subtitle = "1993-2021",
         x = "Year",
         y = "Annual Inflation (%)"
       ) +
...
```
There is lots of variation in annual inflation changes between countries. However, all three countries experience similar decreases in inflation change during the 2008/2009 recession and increases in inflation change during the COVID-19 pandemic.
...
```
The dataset has 132 rows and 4 columns. Each row represents a specific year for a CPI division in the US. The columns include the country name, the CPI division identifier, the year, and the annual inflation rate.
...
```
cpi_divisions has 12 rows and 2 columns. Each row is a particular CPI division and the columns include the division ID and a description of the division.
...
```{r}
us_inflation_full <- left_join(
  us_inflation,
  cpi_divisions,
  by = c("cpi_division_id" = "id")
)
glimpse(us_inflation_full)
```
The joined dataset has 132 rows and 5 columns. Each row represents a specific year for the particular CPI division in the US. Columns include country, CPI division ID, year, annual inflation rate, and description of the CPI division.
...
```{r}
unique(us_inflation_full$description)
...
```
I chose these divisions because they are major areas of spending for the average household.
...
```{r}
filtered_us_inflation <- us_inflation_full |>
  filter(description %in% divisions_of_interest)
filtered_us_inflation |>
...
```{r}
ggplot(filtered_us_inflation, aes (
  x = year, y = annual_inflation, color = description, shape = description)) +
  geom_point() +
  geom_line() +
  labs(
    title = "Annual Inflation by CPI Division",
    subtitle = "United States, 2011-2022",
    x = "Year",
    y = "Annual Inflation (%)",
    color = "CPI Division",
    shape = "CPI Division"
...
```
Most CPI divisions in this plot experience similar variations. However, Transport is an exception that sees the most extreme highs and lows during this period. Food and non-alcoholic beverages also sees the second lowest low around 2016.
